...
def get_media_info(file_name):
    ''' Execute the ffprobe command to get all media information. '''
    
    cmds = ['/usr/local/bin/ffprobe', '-v', 'error', '-show_format', '-show_streams', file_name.encode('UTF-8')]
...
def parse_ffprobe_output(format_string):
...
def get_info_for_mediaserver(stream_list):
